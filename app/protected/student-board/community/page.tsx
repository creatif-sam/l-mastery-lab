...
export default async function CommunityPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("id, full_name, organization_id, community_points, role")
    .eq("id", user.id)
    .single() as any;
  ...
  let postsQuery = supabase
    ...
    .select(`
      ...
      author:profiles!community_posts_author_id_fkey(id, full_name, role, community_points)
    `)
  ...
  let leaderboardQuery = supabase
    .from("profiles")
    .select("id, full_name, community_points, role")
